...
const Leftsidebar = () => {
  ...
  return (
    <section
      className={`left_sidebar ${audio?.audioUrl ? "h-[calc(100vh-110px)]" : "h-[calc(100vh-5px)]"}`}
    >
        ...
        {sidebarLinks.map((e, i) => {
          ...
          return (
            <Link
              key={i}
              href={e.route}
              className={`flex gap-3 items-venter py-4 max-lg:px-4 justify-start ${isActive && "bg-nav-focus border-r-4 border-[#4CAF50]"}`}
            >
      ...
      <SignedOut>
        <div className="flex-center w-full pb-14 max-lg:px-4 lg:pr-8">
          <Button
            asChild
            className="text-16 w-full bg-[#4CAF50] font-extrabold"
          >
